...
4. **Casa** — recipes, ingredients and later pantry/shopping;
...
It remains an orientation dashboard with compact member cards, short meal context and one prominent planning action. `Planear refeição` now enters the operational plan instead of jumping directly into recommendation generation.
...
`Refeições` is now the real planner rather than a read-only agenda.
...
Empty slots remain visible and provide an Add action. A planned meal can be opened to change:

- Recipe;
- local time;
- location;
- Family participants;
- Person-specific quantities/units.

Removal cancels the planned MealEvent rather than erasing history.

Prepared, served and completed events remain visible but are locked from planning edits.
...
These should become additional `Casa` tabs rather than new unrelated dashboards.
...
3. **Core meal-planning foundation** — current large integration:
   - ingredients;
   - recipes;
...
4. Pantry + planned-Recipe requirements + shopping-list lifecycle — next large integration.
5. Recipe ratings/preferences + planning/recommendation ranking integration.
6. Resume secondary Person analytics/detail expansion.
