...
The proposal response exposes the selected weekly combination, Person-specific portions/scores/explanations, optimization counters and per-slot recommendation engine versions. For review UI it also exposes a compact, server-derived Plan-Fit detail snapshot containing the exact candidate nutrition used for that Person, evaluated rules, guideline results, safety issues and conflicts. It deliberately does not repeat the full NutritionPlan record or original source text for every weekly cell. The browser renders this evidence with the same Plan-Fit assessment component used by direct meal evaluation and does not recalculate Plan-Fit or infer plan targets. If a requested slot has no server-authoritative eligible candidate after practical availability and Plan-Fit evaluation, that slot is returned explicitly as skipped/pending and is not allowed to collapse the search space for otherwise feasible slots. Full internal Plan-Fit orchestration remains server-side.
