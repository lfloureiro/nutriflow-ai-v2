...
def _mandatory_unknown_rule_blocks(result: MealPlanFitRuleRead) -> bool:
    if not result.is_mandatory or result.status not in {"unknown", "not_evaluated"}:
        return False
    # Fail closed only when the current Meal Plan-Fit engine claims the rule is
    # machine-evaluable for this decision. Unsupported lifestyle, supplement,
    # timing, meal-composition, and other external target types stay visible as
    # partial coverage instead of becoming a universal meal veto.
