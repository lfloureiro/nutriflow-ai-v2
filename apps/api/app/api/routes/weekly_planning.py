...
from app.services.family_meal_plan import MealPlanError, MealPlanSlotConflictError
...
from app.services.weekly_proposal_acceptance import (
    WeeklyProposalStaleError,
    accept_shared_weekly_plan_slot,
)
...
@router.post(
    "/proposals/accept-slot",
    response_model=SharedWeeklyPlanSlotAcceptanceRead,
    status_code=status.HTTP_201_CREATED,
)
def accept_shared_weekly_plan_slot_endpoint(
    family_id: uuid.UUID,
    ...
    family = _family_or_404(db, family_id)
    try:
        return accept_shared_weekly_plan_slot(db, family=family, data=data)
    except WeeklyProposalStaleError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    except (MealPlanSlotConflictError, MealSlotConflictError) as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    except _NOT_FOUND_ERRORS as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
