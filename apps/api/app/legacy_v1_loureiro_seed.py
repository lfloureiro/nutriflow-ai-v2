...
def _quantity_and_unit(row: dict[str, object]) -> tuple[Decimal, str, str | None]:
    ...
    if raw_quantity is None:
        return Decimal(1), raw_unit or "qb", "Quantidade não especificada na v1; importada como 1 qb."
    try:
        quantity = Decimal(str(raw_quantity))
    except InvalidOperation as exc:
        raise LegacyV1LoureiroSeedError("Invalid recipe ingredient quantity in v1 snapshot.") from exc
    if quantity <= 0:
        return Decimal(1), raw_unit or "qb", "Quantidade não positiva na v1; importada como 1 qb."
...
def _replace_recipe_ingredients(
    ...
    *,
    ...
    if recipe_ids:
        session.execute(delete(RecipeIngredient).where(RecipeIngredient.recipe_id.in_(recipe_ids)))
        session.execute(
            delete(RecipeCompositionSnapshot).where(
                RecipeCompositionSnapshot.recipe_id.in_(recipe_ids),
                RecipeCompositionSnapshot.calculation_version == OLD_SYNTHETIC_CALCULATION_VERSION,
            )
        )
        session.flush()
    ...
    for row in rows:
        ...
        recipe_legacy_id = _as_int(row.get("recipe_id"), field="recipe ingredient recipe id")
        ...
        session.add(
            RecipeIngredient(
                id=uuid.uuid5(LOUREIRO_NAMESPACE, f"recipe-ingredient:{legacy_row_id}"),
                recipe_id=recipe.id,
...
def seed_loureiro_v1_snapshot(
    ...
    *,
    ...
    if not any(
        ...
        raise LegacyV1LoureiroSeedError("The pinned v1 snapshot does not contain Família Loureiro.")

    family = _ensure_family(session)
    people = _ensure_people(session, family, _rows(data, "family_members"))
    ingredients = _ensure_ingredients(session, _rows(data, "ingredients"))
    recipes = _ensure_recipes(session, _rows(data, "recipes"))
    _replace_recipe_ingredients(
        session,
        _rows(data, "recipe_ingredients"),
        recipes=recipes,
