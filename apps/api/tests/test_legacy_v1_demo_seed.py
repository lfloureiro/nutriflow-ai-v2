...
def test_legacy_v1_demo_catalog_is_idempotent_and_keeps_missing_evidence(
    db_session: Session,
) -> None:
    ...
    assert all(recipe.latest_composition.energy_kcal is None for recipe in legacy_recipes)
    assert all(
        recipe.nutrition_issues
